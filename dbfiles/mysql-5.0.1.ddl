...
--  Against: null@offline:mysql?changeLogFile=/Users/dragusa/temp/other_domibus/domibus/Domibus-MSH-db/target/liquibase/changelog-5.0.1.mysql
...
ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN RULE_NAME VARCHAR(255) COMMENT 'Name of the rule used for dispatching';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN SENT datetime COMMENT 'DateTime when the message had been dispatched';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN FAILED datetime COMMENT 'DateTime when the message had failed';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN SEND_ATTEMPTS INT COMMENT 'Number of attempts sent';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN SEND_ATTEMPTS_MAX INT COMMENT 'Number of attempts maximal';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN NEXT_ATTEMPT datetime COMMENT 'DateTime for the next attempt';

ALTER TABLE WS_PLUGIN_TB_BACKEND_MSG_LOG MODIFY COLUMN SCHEDULED BIT(1) COMMENT 'true if the backend message is already scheduled to be sent';
...
ALTER TABLE TB_D_TIMEZONE_OFFSET MODIFY COLUMN NEXT_ATTEMPT_TIMEZONE_ID VARCHAR(255) COMMENT 'Time zone ID on the application server to use when converting the next attempt for displaying it to the user';

ALTER TABLE TB_D_TIMEZONE_OFFSET MODIFY COLUMN NEXT_ATTEMPT_OFFSET_SECONDS INT COMMENT 'Offset in seconds of the time zone on the application server to use when converting the next attempt for displaying it to the user';
...
CREATE TABLE TB_USER_MESSAGE (ID_PK BIGINT AUTO_INCREMENT NOT NULL, MESSAGE_ID VARCHAR(255) NULL, REF_TO_MESSAGE_ID VARCHAR(255) NULL, CONVERSATION_ID VARCHAR(255) NULL, SOURCE_MESSAGE BIT(1) NULL COMMENT 'true if the message is a SourceMessage', MESSAGE_FRAGMENT BIT(1) NULL COMMENT 'true if the message is a message fragment', TEST_MESSAGE BIT(1) NULL, EBMS3_TIMESTAMP timestamp NULL COMMENT 'the EBMS3 timestamp present in eb:Messaging/eb:UserMessage/eb:MessageInfo/eb:Timestamp', ACTION_ID_FK BIGINT NULL, AGREEMENT_ID_FK BIGINT NULL, SERVICE_ID_FK BIGINT NULL, MPC_ID_FK BIGINT NULL, FROM_PARTY_ID_FK BIGINT NULL, FROM_ROLE_ID_FK BIGINT NULL, TO_PARTY_ID_FK BIGINT NULL, TO_ROLE_ID_FK BIGINT NULL, CREATION_TIME timestamp DEFAULT (UTC_TIMESTAMP) NOT NULL, CREATED_BY VARCHAR(255) DEFAULT 'DOMIBUS' NOT NULL, MODIFICATION_TIME timestamp NULL, MODIFIED_BY VARCHAR(255) NULL, CONSTRAINT PK_USER_MESSAGE PRIMARY KEY (ID_PK));

ALTER TABLE TB_USER_MESSAGE MODIFY COLUMN SOURCE_MESSAGE BIT(1) COMMENT 'true if the message is a SourceMessage';

ALTER TABLE TB_USER_MESSAGE MODIFY COLUMN MESSAGE_FRAGMENT BIT(1) COMMENT 'true if the message is a message fragment';

ALTER TABLE TB_USER_MESSAGE MODIFY COLUMN EBMS3_TIMESTAMP timestamp COMMENT 'the EBMS3 timestamp present in eb:Messaging/eb:UserMessage/eb:MessageInfo/eb:Timestamp';
...
ALTER TABLE TB_SJ_MESSAGE_HEADER COMMENT = 'Stores the message header details for SplitAndJoin';

ALTER TABLE TB_SJ_MESSAGE_HEADER MODIFY COLUMN BOUNDARY VARCHAR(255) COMMENT 'The multipart boundary value';

ALTER TABLE TB_SJ_MESSAGE_HEADER MODIFY COLUMN START_MULTIPART VARCHAR(255) COMMENT 'The multipart start value';
...
ALTER TABLE TB_SJ_MESSAGE_GROUP COMMENT = 'Stores information about the message group for SplitAndJoin';

ALTER TABLE TB_SJ_MESSAGE_GROUP MODIFY COLUMN MESSAGE_SIZE BIGINT COMMENT 'The message size before compression';

ALTER TABLE TB_SJ_MESSAGE_GROUP MODIFY COLUMN COMPRESSED_MESSAGE_SIZE BIGINT COMMENT 'The size of the message after compression';

ALTER TABLE TB_SJ_MESSAGE_GROUP MODIFY COLUMN REJECTED BIT(1) COMMENT 'True if the message group has been rejected';

ALTER TABLE TB_SJ_MESSAGE_GROUP MODIFY COLUMN EXPIRED BIT(1) COMMENT 'True if the message group has expired';
...
CREATE TABLE TB_USER_MESSAGE_LOG (ID_PK BIGINT NOT NULL, BACKEND VARCHAR(255) NULL, RECEIVED timestamp NOT NULL, ACKNOWLEDGED timestamp NULL, DOWNLOADED timestamp NULL, ARCHIVED timestamp NULL, EXPORTED timestamp NULL, FAILED timestamp NULL, RESTORED timestamp NULL, DELETED timestamp NULL, NEXT_ATTEMPT timestamp NULL, FK_TIMEZONE_OFFSET BIGINT NULL, SEND_ATTEMPTS INT NULL, SEND_ATTEMPTS_MAX INT NULL, SCHEDULED BIT(1) NULL COMMENT 'true if the message is already scheduled to be sent', VERSION INT DEFAULT 0 NOT NULL COMMENT 'Used for optimistic locking mechanism', MESSAGE_STATUS_ID_FK BIGINT NULL, MSH_ROLE_ID_FK BIGINT NOT NULL, NOTIFICATION_STATUS_ID_FK BIGINT NULL, CREATION_TIME timestamp DEFAULT (UTC_TIMESTAMP) NOT NULL, CREATED_BY VARCHAR(255) DEFAULT 'DOMIBUS' NOT NULL, MODIFICATION_TIME timestamp NULL, MODIFIED_BY VARCHAR(255) NULL, PROCESSING_TYPE VARCHAR(255) NULL, CONSTRAINT PK_MESSAGE_LOG PRIMARY KEY (ID_PK));

ALTER TABLE TB_USER_MESSAGE_LOG MODIFY COLUMN SCHEDULED BIT(1) COMMENT 'true if the message is already scheduled to be sent';

ALTER TABLE TB_USER_MESSAGE_LOG MODIFY COLUMN VERSION INT COMMENT 'Used for optimistic locking mechanism';
...
CREATE TABLE TB_SIGNAL_MESSAGE (ID_PK BIGINT NOT NULL, SIGNAL_MESSAGE_ID VARCHAR(255) NULL, REF_TO_MESSAGE_ID VARCHAR(255) NULL, EBMS3_TIMESTAMP timestamp NULL COMMENT 'the EBMS3 timestamp present in eb:Messaging/eb:UserMessage/eb:MessageInfo/eb:Timestamp', CREATION_TIME timestamp DEFAULT (UTC_TIMESTAMP) NOT NULL, CREATED_BY VARCHAR(255) DEFAULT 'DOMIBUS' NOT NULL, MODIFICATION_TIME timestamp NULL, MODIFIED_BY VARCHAR(255) NULL, CONSTRAINT PK_SIGNAL_MESSAGE PRIMARY KEY (ID_PK));

ALTER TABLE TB_SIGNAL_MESSAGE MODIFY COLUMN EBMS3_TIMESTAMP timestamp COMMENT 'the EBMS3 timestamp present in eb:Messaging/eb:UserMessage/eb:MessageInfo/eb:Timestamp';
...
ALTER TABLE TB_CERTIFICATE MODIFY COLUMN ALERT_IMM_NOTIFICATION_DATE timestamp COMMENT 'Alert imminent notification date';

ALTER TABLE TB_CERTIFICATE MODIFY COLUMN ALERT_EXP_NOTIFICATION_DATE timestamp COMMENT 'Alert expired notification date';
...
ALTER TABLE TB_CERTIFICATE_AUD MODIFY COLUMN ALERT_IMM_NOTIFICATION_DATE timestamp COMMENT 'Alert imminent notification date';

ALTER TABLE TB_CERTIFICATE_AUD MODIFY COLUMN ALERT_EXP_NOTIFICATION_DATE timestamp COMMENT 'Alert expired notification date';
...
ALTER TABLE TB_COMMAND MODIFY COLUMN SERVER_NAME VARCHAR(255) COMMENT 'The target server name';
...
ALTER TABLE TB_COMMAND_PROPERTY MODIFY COLUMN FK_COMMAND BIGINT COMMENT 'Reference to the parent command';
...
ALTER TABLE TB_ENCRYPTION_KEY MODIFY COLUMN KEY_USAGE VARCHAR(255) COMMENT 'The encryption usage: PAYLOAD';

ALTER TABLE TB_ENCRYPTION_KEY MODIFY COLUMN SECRET_KEY LONGBLOB COMMENT 'The secret key used for symmetric encryption';

ALTER TABLE TB_ENCRYPTION_KEY MODIFY COLUMN INIT_VECTOR LONGBLOB COMMENT 'The initialization vector used for encryption';
...
ALTER TABLE TB_VERSION COMMENT = 'Stores Domibus version and build time';
...
ALTER TABLE TB_PM_MPC MODIFY COLUMN RETENTION_SENT INT COMMENT 'Retention time for sent messages (both send success and send failure)';

ALTER TABLE TB_PM_MPC MODIFY COLUMN DELETE_MESSAGE_METADATA BIT(1) COMMENT 'Specifies whether the message metadata should be deleted or not.';

ALTER TABLE TB_PM_MPC MODIFY COLUMN MAX_BATCH_DELETE INT COMMENT 'Maximum number of DOWNLOADED/UNDOWNLOADED/SENT messages to be deleted in one batch';
...
ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN NAME VARCHAR(255) COMMENT 'Splitting configuration name';

ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN FRAGMENT_SIZE INT COMMENT 'The fragment size';

ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN COMPRESSION BIT(1) COMMENT 'True if compression will be used';

ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN COMPRESSION_ALGORITHM VARCHAR(255) COMMENT 'Compression algorithm to use for compressing the SourceMessage';

ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN JOIN_INTERVAL INT COMMENT 'The join interval';

ALTER TABLE TB_PM_SPLITTING MODIFY COLUMN FK_BUSINESSPROCESS BIGINT COMMENT 'Foreign key to the business process';
...
CREATE TABLE TB_EARCHIVE_BATCH (ID_PK BIGINT AUTO_INCREMENT NOT NULL, BATCH_ID VARCHAR(255) NULL, ORIGINAL_BATCH_ID VARCHAR(255) NULL COMMENT 'The original BATCH_ID from which this batch was generated/re-exported', REEXPORTED BIT(1) DEFAULT 0 NOT NULL COMMENT 'true if this batch messages were re-exported by a new batch', REQUEST_TYPE VARCHAR(255) NULL, BATCH_STATUS VARCHAR(255) NULL, DATE_REQUESTED timestamp NULL, LAST_PK_USER_MESSAGE BIGINT NULL, FIRST_PK_USER_MESSAGE BIGINT NULL, ERROR_CODE VARCHAR(255) NULL, ERROR_DETAIL VARCHAR(255) NULL, MANIFEST_CHECK_SUM VARCHAR(255) NULL, BATCH_SIZE INT NULL, STORAGE_LOCATION VARCHAR(255) NULL, CREATION_TIME timestamp DEFAULT (UTC_TIMESTAMP) NOT NULL, CREATED_BY VARCHAR(255) DEFAULT 'DOMIBUS' NOT NULL, MODIFICATION_TIME timestamp NULL, MODIFIED_BY VARCHAR(255) NULL, CONSTRAINT PK_EARCHIVE_BATCH PRIMARY KEY (ID_PK));

ALTER TABLE TB_EARCHIVE_BATCH MODIFY COLUMN ORIGINAL_BATCH_ID VARCHAR(255) COMMENT 'The original BATCH_ID from which this batch was generated/re-exported';

ALTER TABLE TB_EARCHIVE_BATCH MODIFY COLUMN REEXPORTED BIT(1) COMMENT 'true if this batch messages were re-exported by a new batch';
...
ALTER TABLE TB_LOCK COMMENT = 'Stores keys used for locking/synchronizing in cluster';
